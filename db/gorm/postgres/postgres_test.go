...
func TestInit(t *testing.T) {
	...
	t.Run("驱动不匹配", func(t *testing.T) {
		...
		if err == nil || !strings.Contains(err.Error(), "数据库驱动只支持 postgresql") {
	...
	t.Run("配置校验失败", func(t *testing.T) {
		_, err := Init(&orm.Config{Driver: orm.DriverPostgresql}, nopWriter{}, "test")
		if err == nil {
	...
	t.Run("非法 DSN 连接失败", func(t *testing.T) {
		...
		_, err := Init(&orm.Config{Driver: orm.DriverPostgresql, Dsn: "bad-dsn"}, nopWriter{}, "test")
		if err == nil {
