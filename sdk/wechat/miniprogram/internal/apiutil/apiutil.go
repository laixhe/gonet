// Package apiutil 提供微信小程序接口调用共用的请求封装与错误类型。
package apiutil
...
func newRequest(ctx context.Context, client *resty.Client, path string, query map[string]string, body any) *resty.Request {
	...
	// 原生 encoding/json 会对 & < > 做 HTML 转义,微信侧部分参数(如 scene/query)会因此解析错误,统一禁用
...
// PostBinary 发起 POST 请求,成功时返回原始二进制(如小程序码图片),失败时解析 JSON 错误。
