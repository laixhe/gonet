...
func (c *Config) Check() error {
	...
	if c.AccessKeyID == "" {
		return errors.New("没有短信访问密钥 access_key_id 配置")
	}
	if c.AccessKeySecret == "" {
		return errors.New("没有短信访问密钥 access_key_secret 配置")
	}
	if c.Endpoint == "" {
		return errors.New("没有短信访问域名 endpoint 配置")
	}
	if len(c.Sends) == 0 {
...
func Init(config *Config) (*SClient, error) {
