...
func (s *SdkAliyunImageSearch) SearchImage(body io.Reader) ([]*imageSearchClient.SearchImageByPicResponseBodyAuctions, error) {
	...
	resp, err := s.client.SearchImageByPicAdvance(request, runtimeObject)
	if err != nil {
		return nil, err
	}
	if resp.Body == nil || resp.Body.Success == nil || !tea.BoolValue(resp.Body.Success) || resp.Body.Code == nil || tea.Int32Value(resp.Body.Code) != 0 {
		if resp.Body.Msg != nil && tea.StringValue(resp.Body.Msg) != "" {
			return nil, errors.New("aliyun image search pic fail: " + tea.StringValue(resp.Body.Msg))
		}
		return nil, errors.New("aliyun image search pic fail")
	}
